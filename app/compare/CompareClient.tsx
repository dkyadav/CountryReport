          ...
          <div className="card p-6 overflow-x-auto">
            ...
            <table className="w-full text-sm text-left min-w-[500px]">
              ...
              <tbody className="divide-y divide-slate-700/50">
                {[
                  { label: 'GDP Rank (Asia)', fn: (c: CountryData) => `#${c.gdpRank}` },
                  { label: `GDP ${latestYear}`, fn: (c: CountryData) => c.gdpLatest >= 1000 ? `$${(c.gdpLatest / 1000).toFixed(1)}T` : `$${c.gdpLatest}B` },
                  { label: `Per Capita ${latestYear}`, fn: (c: CountryData) => `$${getLatest(c.perCapitaIncome).toLocaleString()}` },
                  { label: `Exports ${latestYear}`, fn: (c: CountryData) => `$${getLatest(c.exports)}B` },
                  { label: `Imports ${latestYear}`, fn: (c: CountryData) => `$${getLatest(c.imports)}B` },
                  { label: 'External Debt', fn: (c: CountryData) => `$${getLatest(c.externalDebt)}B` },
                  { label: `Inflation ${latestYear}`, fn: (c: CountryData) => `${getLatest(c.inflationRate)}%` },
                  { label: `Unemployment ${latestYear}`, fn: (c: CountryData) => `${getLatest(c.unemploymentRate)}%` },
                  { label: `FDI Inflow ${latestYear}`, fn: (c: CountryData) => `$${getLatest(c.fdiInflow)}B` },
                  { label: 'Currency (1 USD)', fn: (c: CountryData) => `${getLatest(c.currencyVsUSD)} ${c.currencyCode}` },
                ].map((row) => (
